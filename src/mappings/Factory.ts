...
import { FACTORY_ADDRESS, ZERO_BI, ONE_BI, ZERO_BD, ADDRESS_ZERO } from '../common/utils/constants'
import {
  Factory,
  Bundle,
  Pool,
...
import { WHITELIST_TOKENS } from '../common/price/price';
...
export function handlePoolCreated(event: PoolCreated): void {
  ...
  let token0 = NewToken.load(event.params.token0.toHexString())
  let token1 = NewToken.load(event.params.token1.toHexString())

  // fetch info if null
  if (token0 === null) {
    token0 = new NewToken(event.params.token0.toHexString())
  ...
  if (token1 === null) {
    token1 = new NewToken(event.params.token1.toHexString())
